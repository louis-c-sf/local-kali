...
let ElsaWorkflowInstanceViewerScreen = class {
    ...
    this.renderActivityPerformanceMenu = () => {
      ...
      const renderFault = () => {
        if (!activityStats.fault)
          return;
        return index.h("elsa-workflow-fault-information", { workflowFault: this.workflowInstance.fault, faultedAt: this.workflowInstance.faultedAt });
      };
      const renderPerformance = () => {
        if (!!activityStats.fault)
          return;
        return index.h("elsa-workflow-performance-information", { activityStats: activityStats });
      };
  ...
  mapActivityModel(activityBlueprint, workflowInstance) {
    ...
    const properties = collection.collection.map(activityBlueprint.inputProperties.data, (value, key) => {
  ...
  onShowWorkflowSettingsClick() {
    eventBus.eventBus.emit(axiosMiddleware_esm.EventTypes.ShowWorkflowSettings);
  }
  onRecordSelected(e) {
  ...
  render() {
    const descriptors = store.state.activityDescriptors;
...
dashboard.Tunnel.injectProps(ElsaWorkflowInstanceViewerScreen, ['serverUrl', 'culture']);
