...
const createHttpClient = async function (baseAddress) {
  ...
  _httpClient = index.axios.create(config);
  const service = new axiosMiddleware_esm.HttpMiddlewareService(_httpClient);
  await eventBus.eventBus.emit(axiosMiddleware_esm.EventTypes.HttpClientConfigCreated, this, { config });
  await eventBus.eventBus.emit(axiosMiddleware_esm.EventTypes.HttpClientCreated, this, { service, _httpClient });
  return _httpClient;
};
  ...
  _elsaClient = {
    ...
    workflowDefinitionsApi: {
      list: async (page, pageSize, versionOptions) => {
        const queryString = {
          version: domain.getVersionOptionsString(versionOptions)
        };
        if (!!page || page === 0)
