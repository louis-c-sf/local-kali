...
import { EventTypes, getVersionOptionsString } from "../models";
let _httpClient = null;
let _elsaClient = null;
export const createHttpClient = async function (baseAddress) {
  if (!!_httpClient)
    return _httpClient;
  const config = {
    baseURL: baseAddress
...
export const createElsaClient = async function (serverUrl) {
  if (!!_elsaClient)
    return _elsaClient;
